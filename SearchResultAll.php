...
global $sql, $availableNumber;
    ...
    $sql = "SELECT * 
            ...
            WHERE (fl.number = C.number) AND (fl.airplane_id = AP.ID) AND (fl.d_date = '$selectdate')
...
if($rowcount == 0){
    echo "<div class='alert alert-info'><strong>Résultats de la recherche: </strong>".$rowcount." result</div>";
}
else{
echo "<div class='alert alert-info'><strong>Résultats de la recherche: </strong>".$rowcount." results</div>";
...
while($row = pg_fetch_array($result)) {
    ...
    echo "<td>" . $row['price'] . "</td>";
        ...
        $seatreserved = "SELECT flightno, classtype, COUNT(*)
                    FROM book B
                    WHERE  B.flightno = '".$row['number']."'AND B.classtype ='".$row['name']."' AND paid=1
                    GROUP BY flightno, classtype";
        $reserved = pg_query($dbconn, $seatreserved);   
        $reservedNumber = pg_fetch_array($reserved);
        
        $capacity = pg_query($dbconn, "SELECT capacity FROM class C WHERE C.number='".$row['number']."' AND C.name= '".$row['name']."'");
